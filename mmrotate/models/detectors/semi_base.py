...
class RotatedSemiBaseDetector(BaseDetector):
    ...
    def __init__(self,
        ...
        """modify"""
        self.iter_count = self.semi_train_cfg.get('iter_count', 0)
        self.burn_in_steps = self.semi_train_cfg.get('burn_in_steps', 6400)
        self.visual = self.semi_train_cfg.get('visual', False)
        self.visual_interval = self.semi_train_cfg.get('visual_interval', 800)
    ...
    def loss_by_pseudo_instances(self,
        ...
        for data_samples in batch_data_samples:
            data_samples.gt_instances.bboxes = RotatedBoxes(data_samples.gt_instances.bboxes)
    ...
    def get_pseudo_instances(
        ...
        for data_samples, results in zip(batch_data_samples, results_list):
            # notes that the results.pred_instances and data_samples.pred_instances share the same memory address
            data_samples.gt_instances = copy.deepcopy(results.pred_instances)
            data_samples.gt_instances.bboxes = bbox_project(
                data_samples.gt_instances.bboxes,
                torch.from_numpy(data_samples.homography_matrix).inverse().to(
                    self.data_preprocessor.device), data_samples.ori_shape)
            # data_samples.pred_instances.bboxes = bbox_project(
            #     data_samples.pred_instances.bboxes,
            #     torch.from_numpy(data_samples.homography_matrix).inverse().to(
            #         self.data_preprocessor.device), data_samples.ori_shape)
    ...
    def project_pseudo_instances(self, batch_pseudo_instances: SampleList,
        ...
        for pseudo_instances, data_samples in zip(batch_pseudo_instances,
            ...
            # data_samples.pred_instances = pseudo_instances.pred_instances
            data_samples.gt_instances.bboxes = bbox_project(
                data_samples.gt_instances.bboxes,
