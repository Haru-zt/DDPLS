...
sup_pipeline = [
    ...
    dict(
        type='mmdet.Pad', size_divisor=32,
...
strong_pipeline = [
    dict(type='RandomApply',
         transforms=dict(type='mmpretrain.ColorJitter',brightness=0.4, contrast=0.4, saturation=0.4, hue=0.1),
         prob=0.8),
    dict(type='mmpretrain.RandomGrayscale', prob=0.2, keep_channels=True),
    dict(type='mmpretrain.GaussianBlur', radius=None, prob=0.5,
         magnitude_level=1.9, magnitude_range=[0.1, 2.0], magnitude_std='inf', total_level=1.9),
    # dict(type='mmpretrain.RandomErasing', erase_prob=0.7, min_area_ratio=0.0005, max_area_ratio=0.002,
    #      aspect_range=(0.3, 0.33), mode='rand'),
    # dict(type='mmpretrain.RandomErasing', erase_prob=0.5, min_area_ratio=0.0002, max_area_ratio=0.002,
    #      aspect_range=(0.1, 6.), mode='rand'),
    # dict(type='mmpretrain.RandomErasing', erase_prob=0.3, min_area_ratio=0.0002, max_area_ratio=0.002,
    #      aspect_range=(0.05, 8.), mode='rand'),
    dict(
        type='mmdet.Pad', size_divisor=32,
        pad_val=dict(img=(114, 114, 114))),
    dict(
...
labeled_dataset = dict(
    ...
    filter_cfg=dict(filter_empty_gt=True),#todo ?
